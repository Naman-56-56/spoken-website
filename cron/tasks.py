...
def process_test_attendance(test_id):
    ...
    # If no FossMdlCourses record exists, cannot process test attendance
    if fossmdlcourse is None:
        meta_update(
            status="failed",
            message=f"No FossMdlCourses mapping found for FOSS ID {test.foss_id}"
        )
        return
