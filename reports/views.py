...
def events_training_csv(request):
    ...
    # header
    writer.writerow(['State', 'City', 'Institution', 'Institution Type', 'FOSS', 'Department', 'Type', 'Organiser', 'Date', 'Participants'])
    ...
    for record in collection.qs:
        writer.writerow([
            ...
            record.training_planner.academic.institution_name,
            record.training_planner.academic.institution_type.name,
            record.course.foss.foss,
            record.department,
            record.course.category_name(),
...
def events_test_csv(request):
    ...
    for record in collection.qs:
        writer.writerow([
            to_text(record.academic.state.name),
            to_text(record.academic.city.name),
...
def elibrary(request):
    ...
    for tr in trs:
        ...
        writer.writerow([tr.video, user_name, domain_reviewer, 'NMEICT', '', tr.created, '', '', publish_date, videourl, '', '', '', tr.outline, '', vdurwithsize, 'video/webm', tr.language.name, '', tr.tutorial_detail.foss.foss, '', '', tr.common_content.keyword,
